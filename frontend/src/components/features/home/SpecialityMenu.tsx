...
export const SpecialityMenu = () => {
  ...
  const {
    ...
    isError,
    refetch
  } = useGetDepartment(1, 10, "");

  const departments = data?.departments ?? [];

  const showSkeleton =
    isPending || (isFetching && departments.length === 0);
    ...
    <section className="py-12 md:py-16 lg:py-20 bg-white">
      <div className="container">
        <div className="text-center mb-10 md:mb-16">
          ...
          <div className="w-16 md:w-20 h-1 bg-linear-to-r from-primary to-secondary rounded-full mx-auto" />
        </div>

        {showSkeleton && <SpecialityMenuSkeleton />}

        {!showSkeleton && isError && departments.length === 0 && (
          <div className="min-h-[250px] flex flex-col items-center justify-center text-center">
            <p className="text-secondary mb-4">
              {t("departments:failedToLoad")}
            </p>
        ...
        {!showSkeleton && departments.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
            {departments.map((item: Department) => (
              ...
              >
                <div className="absolute -top-6 -left-6 w-12 h-12 bg-primary/10 rounded-full group-hover:bg-primary/20 transition-all duration-500" />
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <div className="w-12 h-12 md:w-14 md:h-14 bg-linear-to-br from-primary to-secondary rounded-xl md:rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300 shrink-0">
                    <img
                      src={item.icon || "/src/assets/icons/headset.svg"}
                      alt={
                        i18n.language === "ar"
                          ? item.name_ar
                          : item.name_en
